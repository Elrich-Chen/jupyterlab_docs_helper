  ...
  activate: (
    ...
    app.commands.addCommand(command, {
       ...
       execute: async () => {
          ...
          if (nb.activeCellIndex < 0 && nb.widgets.length > 0) nb.activeCellIndex = 0;
...
palette.addItem({ command: command, category: 'Docs Helper' });
  ...
  const src = typeof sm.getSource === 'function' ? sm.getSource() : (sm.source ?? '');
...
`;
  ...
  const placeholderText = '### ⏳ Generating explanation…\n\nThis will be replaced automatically when ready.';
  if (typeof phsm.setSource === 'function') phsm.setSource(placeholderText);
  else phsm.source = placeholderText;
  console.log('[docs-helper] placeholder inserted at index', placeholderIndex);
...
function stripFences(s: string): string {
  ...
  if (t.startsWith('```') && t.endsWith('```')) {
    return t.replace(/^```[a-zA-Z0-9_-]*\s*/, '').replace(/\s*```$/, '').trim();
  }
  return t;
}

// 3) Wait for outputs to contain non-empty text (handles streaming)
function waitForTextFromOutputs(outputs: any, timeoutMs = 30000): Promise<string> {
